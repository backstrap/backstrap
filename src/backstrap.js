...
(function (context) {
  ...
  var backstrap = function backstrap() {
	...
	var classlist = {};
	// We only get Bootstrap button behavior on actual button tags.
	if (arguments[0] === 'button') {classlist.btn = true;}
        for(var key in arg) {
		  ...
		  // The rest are special handling for Bootstrap keys
	          case 'size':
		    if (value in sizeMap) {
	            	classlist[(classlist.btn ? 'btn-' : 'text-') + sizeMap[value]] = true;
		    } else {
		    ...
		    break;
	          ...
	          case 'context':
	            classlist[(classlist.btn ? 'btn-' : 'text-') + value] = true;
		    break;
